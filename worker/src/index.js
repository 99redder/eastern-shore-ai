...
/**
 * POST /api/ask-k — AI assistant for Survival Node product questions (public endpoint)
...
async function handleAskK(request, env, corsHeaders) {
  ...
  if (!userMessage) {
    return json({ ok: false, error: 'Missing message' }, 400, corsHeaders);
  }

  // Rate limit: max message length
  if (userMessage.length > 1000) {
    return json({ ok: false, error: 'Message too long. Please keep it under 1000 characters.' }, 400, corsHeaders);
  }

  // Prompt-injection hardening: reject messages with system/instruction override attempts
  ...
  // Check for API key
  const apiKey = env.ASKK_API_KEY;
  if (!apiKey) {
    return json({ ok: false, error: 'AI assistant not configured' }, 500, corsHeaders);
  }

  // Normalize base URL to /chat/completions
  let baseUrl = (env.ASKK_BASE_URL || 'https://api.openai.com/v1').toString().trim();
  ...
  const model = env.ASKK_MODEL || 'gpt-4o-mini';

  // Build context string from page state
  let contextNote = '';
  if (pageContext.visibleSections?.length) {
    contextNote += `\n[Visible sections: ${pageContext.visibleSections.join(', ')}]`;
  }
  if (pageContext.expandedFAQs?.length) {
    contextNote += `\n[Expanded FAQs: ${pageContext.expandedFAQs.join(', ')}]`;
  }
  if (pageContext.selectedUpgrades?.length) {
    contextNote += `\n[Selected upgrades: ${pageContext.selectedUpgrades.join(', ')}]`;
  }

  // Build messages array
  const messages = [
    { role: 'system', content: ASKK_SURVIVAL_NODE_KNOWLEDGE }
  ];

  // Add conversation history
  for (const msg of history) {
    if (msg.role === 'user' || msg.role === 'assistant') {
      messages.push({ role: msg.role, content: (msg.content || '').toString().slice(0, 2000) });
    }
  }
  ...
  messages.push({
    role: 'user',
    content: contextNote ? `${userMessage}\n\n[Page context:${contextNote}]` : userMessage
  });
  ...
  try {
    ...
    if (!res.ok) {
      console.error('Ask K API error:', res.status, resData);
      return json({ ok: false, error: 'AI assistant temporarily unavailable' }, 502, corsHeaders);
    }

    let reply = resData.choices?.[0]?.message?.content || resData.content?.[0]?.text || '';

    // Strip <think> blocks if present (some models use this)
    reply = reply.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();

    if (!reply) {
      return json({ ok: false, error: 'No response from AI assistant' }, 502, corsHeaders);
    }

    return json({ ok: true, reply }, 200, corsHeaders);
  } catch (err) {
    console.error('Ask K fetch error:', err);
    return json({ ok: false, error: 'AI assistant temporarily unavailable' }, 502, corsHeaders);
  }
}
...
async function handleAskKEscalate(request, env, corsHeaders) {
  ...
  const payload = {
    text: `**Ask K Escalation Request**\n\nPage: Survival Node (node.html)\n\n**Conversation:**\n${conversationText || '(no conversation)'}\n\n**Contact:** Call (302) 907-9162 or check email`
  };
  ...
  try {
    ...
    if (!res.ok) {
      console.error('Escalation webhook failed:', res.status);
    }
