// ===== API URL Constants =====
    ...
    function renderOrderEmailPreview() {
      ...
      const bodyHtml = blocks.map((part) => `<p style="margin:0 0 20px;color:#d8dce8;white-space:pre-wrap;line-height:1.7;">${escHtml(part)}</p>`).join('') || '<p style="margin:0 0 20px;color:#d8dce8;">&nbsp;</p>';
      const detailLines = [
      ...
      const batteryHtml = !isDelivered && !isReview && (batteryTestNote || batteryTestImageKey)
        ? `<div style="margin:20px 0;padding:16px 20px;background:#101923;border:1px solid #26364a;border-left:4px solid #00e5ff;border-radius:6px;"><div style="font-weight:700;color:#00e5ff;margin-bottom:8px;">Battery Test Results</div>${batteryTestNote ? `<div style="color:#d8dce8;white-space:pre-wrap;">${escHtml(batteryTestNote)}</div>` : ''}${batteryTestImageKey ? `<div style="margin-top:10px;"><a href="https://services.easternshore.ai/api/orders/battery-image?key=${encodeURIComponent(batteryTestImageKey)}" style="color:#9ed0ff;">View AccuBattery results &rarr;</a></div>` : ''}</div>`
        : '';
      const shippingGuideHtml = isDelivered
        ? `<div style="margin:28px 0;padding:16px 20px;background:#101923;border:1px solid #26364a;border-left:4px solid #c850ff;border-radius:6px;color:#d8dce8;"><div style="font-weight:700;color:#d8c2ff;margin-bottom:8px;">Getting Started</div><div style="line-height:1.6;">The User Guide is an app on the main screen of the phone — just tap it to open anytime.<br><br>Your case also includes two inserts: one walks you through the <strong>first steps</strong> to get set up, and the other covers <strong>how to deploy</strong> when you need it.<br><br>You can also view the full User Guide on the web here: <a href="https://www.easternshore.ai/userguide.html" style="color:#9ed0ff;font-weight:700;">https://www.easternshore.ai/userguide.html</a></div></div>`
        : '';
      const googleReviewHtml = isReview
        ? `<div style="margin:24px 0;text-align:center;"><a href="https://www.easternshore.ai/review" style="display:inline-block;background:#c850ff;background-image:linear-gradient(145deg,#ff4fd8,#c850ff);color:#ffffff;text-decoration:none;padding:14px 28px;border-radius:6px;font-weight:700;font-size:16px;">Leave a Google Review &#9733;</a></div>`
        : '';
      ...
      orderEmailPreviewEl.innerHTML = `<div style="font-family:Arial,sans-serif;background:#0a0b10;padding:24px 12px;color:#d8dce8;color-scheme:dark;"><div style="max-width:700px;margin:0 auto;background:#141620;border:1px solid #2a2d42;border-radius:12px;overflow:hidden;"><div style="height:4px;background:#00e5ff;background-image:linear-gradient(90deg,#00e5ff,#c850ff,#ff4fd8);font-size:0;line-height:0;">&nbsp;</div><div style="padding:24px;background:#0c0d14;border-bottom:1px solid #222438;color:#eaf1ff;"><div style="font-size:12px;letter-spacing:1.8px;text-transform:uppercase;color:#00e5ff;font-weight:700;">Eastern Shore AI</div><h1 style="margin:8px 0 0;font-size:25px;line-height:1.25;color:#eaf1ff;">${escHtml(subject || 'Preview')}</h1><div style="margin-top:9px;font-size:14px;color:#a5b1c8;">${escHtml(preheader)}</div></div><div style="padding:24px;background:#141620;"><div style="margin:0 0 22px;padding:16px 18px;background:#10121a;border:1px solid #222438;border-left:4px solid #c850ff;border-radius:6px;color:#d8dce8;line-height:1.55;">${detailLines}</div>${bodyHtml}${batteryHtml}${trackProxyUrl ? `<div style="margin:24px 0 10px;text-align:center;"><a href="${escHtml(trackProxyUrl)}" style="display:inline-block;background:#c850ff;background-image:linear-gradient(145deg,#ff4fd8,#c850ff);color:#ffffff;text-decoration:none;padding:13px 22px;border-radius:6px;font-weight:700;">${escHtml(trackButtonLabel)}</a><div style="margin-top:9px;font-size:11px;color:#7a7e96;">Tracking opens the official carrier website.</div></div>` : ''}${shippingGuideHtml}${googleReviewHtml}</div><div style="padding:18px 24px;border-top:1px solid #222438;background:#0c0d14;color:#a5b1c8;font-size:13px;line-height:1.5;text-align:center;"><div style="margin-bottom:10px;color:#a5b1c8;"><strong style="color:#d8dce8;">Why you received this:</strong> ${escHtml(transactionContext)}</div><div style="margin-bottom:14px;padding:10px 12px;background:#10121a;border:1px solid #222438;border-radius:5px;color:#a5b1c8;font-size:12px;"><strong style="color:#d8c2ff;">Security reminder:</strong> ${escHtml(securityReminder)}</div><strong style="color:#d8dce8;">Eastern Shore AI, LLC</strong><span style="color:#7a7e96;"> &bull; </span>Fruitland, Maryland<div style="margin-top:6px;"><a href="mailto:contact@easternshore.ai" style="color:#00e5ff;">contact@easternshore.ai</a><span style="color:#7a7e96;"> &bull; </span><a href="tel:+13029079162" style="color:#00e5ff;">(302) 907-9162</a></div><div style="margin-top:6px;"><a href="https://www.easternshore.ai" style="color:#00e5ff;">www.easternshore.ai</a></div><p style="margin:8px 0 0;font-size:11px;line-height:1.45;color:#7a7e96;">Privacy: We use your contact information only to fulfill your order and send related service communications.</p></div></div></div>`;
    }
